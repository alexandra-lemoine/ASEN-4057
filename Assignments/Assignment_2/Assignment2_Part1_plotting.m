...
%% Grid Search
stepsize = 25;                   % Step Size of the Grid Search
...
for i = 1:2:length(y(:,8))
    axis([-0.5*10^8 3*10^8 -0.1*10^8 4*10^8]) 
    %plot path of spacecraft
    p_s = plot3(y(1:i,7),y(1:i,8),zeros(1,length(y(:,8))),'m');
    %plot path of moon
    p_m = plot(y(1:i,9),y(1:i,10),zeros(1,length(y(:,8))),'k');
    % Plot Trajectory of Spacecraft
    s_p = plot(y(i,7) + x_S_p, y(i,8) + y_S_p,zeros(1,length(y(:,8))),'b');
    %Plot Trajectory of the Moon
    m_p = plot(y(i,9) + x_M_p ,y(i,10) + y_M_p,zeros(1,length(y(:,8))),'k');
